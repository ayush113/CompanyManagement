...
def leads(request):
    with connection.cursor() as curr:
        curr.execute("select leads.customer_id,customer_name,proposed_project_info,time_required,cost,leads.info from customer,leads where point_of_contact=%s and leads.customer_id=customer.customer_id",[request.user.id])
